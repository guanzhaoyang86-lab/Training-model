...
RESIDUAL_POOL_SAMPLING_RATIOS="${RESIDUAL_POOL_SAMPLING_RATIOS:-false_negative=0.60,boundary_error=0.20,false_positive=0.10,correct_abnormal=0.10,correct_normal=0.00}"

REWARD_POINT_WEIGHT="${REWARD_POINT_WEIGHT:-0.60}"
REWARD_EVENT_WEIGHT="${REWARD_EVENT_WEIGHT:-0.00}"
REWARD_IOU_WEIGHT="${REWARD_IOU_WEIGHT:-0.25}"
REWARD_BOUNDARY_WEIGHT="${REWARD_BOUNDARY_WEIGHT:-0.15}"
REWARD_TYPE_WEIGHT="${REWARD_TYPE_WEIGHT:-0.00}"
...
grpo_cmd=(
  ...
  --num_generations "${RL_NUM_GENERATIONS:-6}"
  --max_new_tokens "${RL_MAX_NEW_TOKENS:-512}"
  --temperature "${RL_TEMPERATURE:-0.7}"
  --top_p "${RL_TOP_P:-0.9}"
  --learning_rate "${RL_LEARNING_RATE:-2e-6}"
  --num_train_epochs "${RL_NUM_TRAIN_EPOCHS:-2}"
  --kl_coef "${RL_KL_COEF:-0.03}"
  --tau_match "${TAU_MATCH:-0.1}"
  --tau_good "${TAU_GOOD:-0.5}"
  --series_length "${SERIES_LENGTH:-256}"
  --max_index "${MAX_INDEX:-255}"
