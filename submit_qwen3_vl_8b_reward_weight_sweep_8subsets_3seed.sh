...
# Reward-weight sweep for residual-GRPO v2.
# Fixed setup:
#   model: Qwen3-VL-8B
...
#
# Each reward config is submitted as an independent batch. The existing
# missing-metrics checker is scheduled after every config/seed batch and
# resubmits any model/setting/subset that did not produce test_metrics.json.
# The underlying workflow deletes model/checkpoint weights after evaluation and
# keeps metrics, logs, manifests, and summaries.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"

RUN_SEEDS="${RUN_SEEDS:-2026 2027 2028}"
BATCH_PREFIX="${BATCH_PREFIX:-qwen3_vl_8b_reward_weight_sweep_8subsets_3seed}"
...
v2_current:0.60:0.00:0.25:0.15:0.00 \
point_080:0.80:0.00:0.15:0.05:0.00 \
point_070:0.70:0.00:0.20:0.10:0.00 \
balanced_050:0.50:0.00:0.30:0.20:0.00 \
iou_heavy:0.45:0.00:0.40:0.15:0.00 \
boundary_heavy:0.45:0.00:0.25:0.30:0.00 \
low_point:0.35:0.00:0.40:0.25:0.00 \
event_restore:0.45:0.15:0.25:0.15:0.00}"
...
echo "Submitting Qwen3-VL-8B residual_v2 reward-weight sweep."
...
for config in $REWARD_CONFIGS; do
  ...
  # Base values are passed for compatibility; residual_v2 overrides use the
  # RESIDUAL_V2_* values above inside the main workflow.
  ...
  for seed in $RUN_SEEDS; do
    ...
    if [[ "$AUTO_RESUBMIT_MISSING" == "1" && "${DRY_RUN:-0}" != "1" ]]; then
      ...
      auto_sbatch_cmd=(
        ...
        "--job-name=ensure-rw-${config_name//_/-}-s${seed}"
...
echo
echo "Submitted reward-weight sweep batches: $submitted_batches"
echo "Expected final metrics: 8 configs x 3 seeds x 8 subsets = 192 test_metrics.json files."
echo "Batch prefix:"
echo "  qwen_vl_all_models_sft_residual_ablation_8subsets_${BATCH_PREFIX}"
echo "After all jobs finish, list result directories with:"
echo "  ls -td $OUTPUT_ROOT/qwen_vl_all_models_sft_residual_ablation_8subsets_${BATCH_PREFIX}_* | head"
